...
uglifyjs eng_2.3.js -m -c -o eng_2.3.min.js   //压缩 =>   混淆,压缩,输出
       ...
       t.getNowTree=function(str,pTree,path,forF){
            ...
            var key={'$_parent':true,'$_p':true,'$_P':true}; 
               while(i<l){
                     a=arr[i]; //路径名
                     if(!key[a]){//向下
                         // 2022-07-20   修正 基于同个bass下的多个bass嵌套的 导致的数据覆盖错误, 只处理显示最后一个 
                         d=p[a]; 
                         if(!d){
                              d={
                                $_parent:p, //父级base数据
...
//   var proto=getProto(Arr,Fun); //获得一个 用于 指向原型 的覆盖Array 的方法 用于触发响应
  var dlf=t.dataOnly;          //纯数据响应
           ...
           * @param {*} o  处于循环中 
           */
          t.jsonCover=function(n,o,f){
               var k,v;
                   //新值覆盖旧值,  旧的值无覆盖时,保留 (所以赋值新数据时,不能缺省,至少要写空值)
                   for(k in n){
          ...
          /** 
           * 2022-07-28
           * 清除循环数据中的不可枚举属性 (防止自定义属性, 重新排列时,不再生效)
           * @param {*} o 
           */
          t.jsonClear=function(o){
               var k,arr=Object.getOwnPropertyNames(o),i=0,l=arr.length;
                   while(i<l){
                      k=arr[i];
                      if(reg.test(k)&&k!=='$_sign')obk(o,k,{enumerable:true});
                      i++;
                   };
          };
           /**
           * array数据覆盖    2022-07-28  (赋值式修改, 取消覆盖式 ,防止array引用对象 失效)
           * @param {*} n  新的array值
           * @param {*} o  旧的array值
           */
           ...
           */
           t.bindKey=function(key,obj,type,path,pos,arrF,del,jbF){
              var value=obj[key],flag, nrr,treeD,domArr,keyStr=path.join('.'),wFun,reV,bF,rf,aF,u;
                   ...
                   obk(obj, key, {
                         ...
                         set:function(s){
                                  ...
                                  //数据响应绑定检查~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
                                   ...
                                   // 从 调用数组方法入手, 开始前flag=true  return ,结束后flag=false; 
                                  //类型 判断 ~~~~~~~~~~~~~~~~~~~~~~~~~
                              var type2= typeLX(s);
                                  if(flag&&value!==s&&limit[type2]){ //类型不匹配时 ,只有 1:string,2:boolean,3:number  基本类型允许相互覆盖
                                        ...
                                        if(type==5){ //5   json 全新的覆盖数据
                                             //  t.dataClear(value,s);
                                             if(arrF){//数组循环中  直接取代旧值 (排序,删除位置发生变化)   
                                                  value=s;
                                                  t.jsonClear(value);
                                             }else{   // 避免引用对象的失效, 新值覆旧值
                                                  t.jsonCover(s,value);                    // 2022-07-28  (赋值式修改, 取消覆盖式)
                                             }; 
                                             ...
                                             //  value.$_zdy(aF); //数组方法绑定
                                              t.arrCover(s,value);                      //2022-07-28  避免引用对象失效,新值覆盖旧值
